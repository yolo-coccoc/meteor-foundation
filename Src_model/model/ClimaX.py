...
class ClimaX_no_time(ClimaX):
    """ClimaX model without lead-time conditioning.
    
    Identical to ClimaX but excludes lead_time embeddings. Inherits all other
    functionality from the parent ClimaX class.
    """
    ...
    def __init__(
        ...
        # Remove the lead_time_embed layer
        del self.lead_time_embed

    def encode_input(self, x: torch.Tensor, variables=None):
        """Encode input variables into transformer tokens.
        
        Args:
            x: Input tensor [B, Vi, H, W]
            variables: Input variable names
    ...
    def forward(self, x, variables=None, out_variables=None):
        """Forward pass through the model.
        ...
        Returns:
            Predicted patches [B, Vo, H, W]
        """
        if variables is None:
            variables = self.variable_names
        
        encoded = self.encode_input(x, variables)
        preds = self.head(encoded)
        preds = self.unpatchify(preds)
